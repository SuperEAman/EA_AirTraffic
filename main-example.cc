...
const float CROSSOVER_P = 0.7;
	...
	int i, iteration, num_iterations, RNG_seed, stop_if_no_change, print_output,
			...
			best_yet_iteration, less_fit_replacement_count = 0, offending_ac_id,
			offending_ac_found, num_iterations_with_constant_fitness = 0,
	...
	fitness_t *child_fitness=new fitness_t[NUM_INDIVIDUALS],
			*parent_fitness=new fitness_t[NUM_INDIVIDUALS], delta_fitness, best_yet_fitness,
	...
	Ordering *parent_ordering = new Ordering[NUM_INDIVIDUALS], *child_ordering = new Ordering[NUM_INDIVIDUALS], *tmp_ordering;
	...
	for (i = 0; i < NUM_INDIVIDUALS; i++) {
		...
		delayed_flights_array[i].delayed_flight_list =
...
//		        return m_PopAll[idx-1];
		delayed_flights_t *a= delayed_flights_array;
		for (int i = 0; i <= NUM_INDIVIDUALS; i += 2){
			int randNum = random_int(0, NUM_INDIVIDUALS - 1);
			int randNum1 = random_int(0, NUM_INDIVIDUALS - 1);
			Ordering *p_pointer ;
			p_pointer= &parent_ordering[randNum];
			Ordering *p_pointer1;
			p_pointer1= &parent_ordering[randNum1];

			crossover(p_pointer, p_pointer1,
							&child_ordering[i], &child_ordering[i+1],
						  0, CROSSOVER_P, ORDER_BASED,
						  delayed_flights_array,
						  delayed_flights_array);
		...
		//child_ordering->mutate_non_enroute_flights(mut_prob);
			...
			if (offending_ac_found) {
				...
				child_ordering[i].move_ac_id_to_top(offending_ac_id);
			}
		}
		// }}}
...

		{
			...
			//---------------------------------------------------------
			for (int i =0 ;i < NUM_INDIVIDUALS; ++i){
					child_fitness[i] = (float) g.greedilySchedule(*child_ordering, false,
							&delayed_list_size,
					...
					if (child_fitness[i] < 0.0) {
						...
						offending_ac_id = (int) (child_fitness[i] * -1.0);
						child_fitness[i] = WORST_FITNESS;
					} else {
						offending_ac_found = 0;
					}
		...
		if (print_output)
			sprintf(output_line, "%6d byf: %4.0f | ", iteration,
					best_yet_fitness);
		...
		for(int i =0; i < NUM_INDIVIDUALS;++i){
				if (child_fitness[i] <= parent_fitness[i]) {
					...
					*tmp_ordering = parent_ordering[i];
					parent_ordering[i] = child_ordering[i];
					child_ordering[i] = *tmp_ordering;

					parent_fitness[i] = child_fitness[i];
				} else {
					delta_fitness = 10 * (child_fitness[i] - parent_fitness[i]);
		...
		if (!(iteration % iterations_per_temperature_change)
				&& (iteration > 0)) {
